...
{
    pthread_barrier_t early_init, late_init;
    ...
    pthread_mutex_t mtx;
    pthread_cond_t cv;
    pthread_t thr; // v8 thread handle
    ...
    Buf snapshot;
    uintptr_t isolate; // for v8_terminate_execution()
...
static void des_bigint(void *arg, const void *p, size_t n, int sign)
{
    ...
    size_t i;
    DesCtx *c;
    uint64_t *a, t, limbs[65]; // +1 to suppress sign extension
...
static void *v8_watchdog(void *arg)
{
    ...
    for (;;) {
        ...
        if (deadline_exceeded(deadline)) {
            v8_terminate_execution(c->isolate);
            break;
        }
    }
...
static void v8_timedwait(Context *c, const uint8_t *p, size_t n,
                         void (*func)(const uint8_t *p, size_t n))
{
    pthread_t thr;
    int r;
...
static void dispatch1(Context *c, const uint8_t *p, size_t n)
{
    assert(n > 0);
    switch (*p) {
    case 'A': return v8_attach(p+1, n-1);
    case 'C': return v8_timedwait(c, p+1, n-1, v8_call);
    case 'E': return v8_timedwait(c, p+1, n-1, v8_eval);
    case 'H': return v8_heap_snapshot();
    case 'I': return v8_idle_notification(p+1, n-1);
    case 'P': return v8_pump_message_loop();
    case 'S': return v8_heap_stats();
    case 'T': return v8_snapshot(p+1, n-1);
    case 'W': return v8_warmup(p+1, n-1);
    case 'L':
        uint8_t b = 0;
        v8_reply(c, &b, 1); // doesn't matter what as long as it's not empty
        return v8_low_memory_notification();
    }
...
void v8_thread_main(Context *c, uintptr_t isolate)
{
    struct timespec deadline;

    c->isolate = isolate;
    pthread_barrier_wait(&c->late_init);
...
static void *v8_thread_start(void *arg)
{
    ...
    c = arg;
    pthread_barrier_wait(&c->early_init);
    v8_once_init();
    ...
    pthread_barrier_destroy(&c->early_init);
    pthread_barrier_destroy(&c->late_init);
...
static VALUE rendezvous_callback_do(VALUE arg)
{
    ...
    return rb_funcall2(func, rb_intern("call"), RARRAY_LEN(args), RARRAY_PTR(args));
}
...
static VALUE context_alloc(VALUE klass)
{
    ...
    if (pthread_cond_init(&c->wd.cv, &cattr))
        goto fail5;
    cause = "pthread_barrier_init";
    if ((r = pthread_barrier_init(&c->early_init, NULL, 2)))
        goto fail6;
    cause = "pthread_barrier_init";
    if ((r = pthread_barrier_init(&c->late_init, NULL, 2)))
        goto fail7;
    cause = "pthread_attr_init";
    if ((r = pthread_attr_init(&tattr)))
        goto fail8;
    pthread_attr_setstacksize(&tattr, 2u<<20); // 2 MiB
    pthread_attr_setdetachstate(&tattr, PTHREAD_CREATE_DETACHED);
    // v8 thread takes ownership of |c|
    cause = "pthread_create";
    r = pthread_create(&c->thr, &tattr, v8_thread_start, c);
...
static VALUE context_heap_stats(VALUE self)
{
    ...
    for (i = 0, n = RARRAY_LEN(a); i < n; i += 2) {
...
static VALUE platform_set_flags(int argc, VALUE *argv, VALUE klass)
{
    ...
    for (i = 0, n = RARRAY_LEN(args); i < n; i += 2) {
...
static VALUE context_initialize(int argc, VALUE *argv, VALUE self)
{
    ...
    while (RARRAY_LEN(a)) {
        ...
        if (!strcmp(s, "ensure_gc_after_idle")) {
            ...
            if (c->idle_gc < 0 || c->idle_gc >= UINT32_MAX)
                rb_raise(rb_eArgError, "bad ensure_gc_after_idle: %ld", c->idle_gc);
        } else if (!strcmp(s, "max_memory")) {
            Check_Type(v, T_FIXNUM);
            c->max_memory = FIX2LONG(v);
            if (c->max_memory < 0 || c->max_memory >= UINT32_MAX)
                rb_raise(rb_eArgError, "bad max_memory: %ld", c->max_memory);
